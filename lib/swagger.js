...
function parseSchema(obj) {
    ...
    if (obj.applications) {
        ...
        for (let i in obj.applications) {
            ...
            if (type == 'object' || type == 'string' || type == 'integer' || type == 'boolean' || type == 'number') {
                oneOf.push({"type": obj.applications[i].name});
            }
            else oneOf.push({
...
function parseTypedef(tags) {
    ...
    for (let i = 1; i < tags.length; i++) {
        if (tags[i].title == 'property') {
            ...
            const required = propName && propName.split('.')[1];
            ...
            if(!details.properties[propName].description && tags[i] && tags[i].description) {
                details.properties[propName].description = tags[i].description;
            }
            if(details.properties[propName].description && details.properties[propName].description.indexOf("=")=== 0) {
                const tmpDefault = details.properties[propName].description.split("=")[1].trim();
                if(details.properties[propName].type === "array") {
                    details.properties[propName].example = [tmpDefault];
                } else if(details.properties[propName].type === "number") {
                    details.properties[propName].example = parseInt(tmpDefault);
                } else if(details.properties[propName].type === "boolean") {
                    details.properties[propName].example = tmpDefault === 'true';
                }
                else {
                    details.properties[propName].example = tmpDefault;
                }
                details.properties[propName].description = "";
            }
        }
    }
